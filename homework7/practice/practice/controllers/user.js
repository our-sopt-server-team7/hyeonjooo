...
module.exports = {
    ...
    updateProfile: async (req, res) => {
        // 데이터 받아오기
        const userIdx = req.decoded.userIdx;//jwt안에 있던 userIdx를 가지고온것임
        const profileImg = req.file.path; //path아니라 location으로수정
        // data check - undefined
        if (profileImg === undefined || !userIdx) {
            return res.status(CODE.OK).send(util.fail(CODE.BAD_REQUEST, "없어~"));
        }
        // image type check
        const type = req.file.mimetype.split('/')[1];//확장자를 가지고온것 req.file에서
